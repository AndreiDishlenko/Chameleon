//
//    (C) 2005, 2009 Aktiv Co. All rights reserved.
...
#define GrdContainerSize    0x3000  // Size of memory allocated for Guardant protected container
...
#define GrdDM_Total         9       // Number of different models
...
#define GrdLM_PerHandle     0x00000001  // Allocate Guardant Net license for each logged in protected container (application copy)
...
#define GrdWMFM_DriverAuto      0x0000  // Use driver automaticaly. Call the dongle by means of driver if it is installed
...
#define GrdECC160_PRIVATE_KEY_SIZE  20  // ECC160 privale key size
...
#define GrdSHA256_CONTEXT_SIZE      0x200   // must be >= sizeof(SHA256_CONTEXT)
...
#define GrdE_gcProhibitCode         72  // Verification of loadable code failed
                                        // Probable reasons: invalid entry point, forbidden insructions
...
#define GrdE_LastError                              97  // Total number of errors
...
typedef struct
{
	...
	BYTE    abyReservedH[0x100 - 0x1A - 0x1C - 0x0E]; // Reserved. For align to 0x100
...
//- Dongle State Flags
#define GrdDSF_RTC_Quality       1 // Real Time Clock Quality error flag
#define GrdDSF_RTC_Battery       2 // Real Time Clock Battery error flag
...
int GRD_API GrdStartupEx( 
		...
		char    *szNetworkClientIniPath,    // Path to gnClient.ini. NULL default place near application executable
		void    *pReserved );               // Reserved Must by NULL
...
// Convert Guardant error code to a text message string
int GRD_API GrdFormatMessage(
		HANDLE  hGrd,           // if != NULL then handle for GrdGetLastError()
...
HANDLE GRD_API GrdCreateHandle(
		HANDLE  hGrd,           // Pointer to memory allocated for protected container
		//                         if NULL, GrdAPI allocates memory for a new protected container by itself
		DWORD   dwMode,         // Create Handle Mode flags. See GrdCHM_XXXXX definition
		void    *pReserved );   // Reserved, must be NULL
...
int GRD_API GrdLock( 
		HANDLE  hGrd,                   // Handle to logged in Guardant protected container
...
int GRD_API GrdWrite(
		...
		void    *pData,         // Buffer for data to be written
		void    *pReserved );   // Reserved, must be NULL
...
int GRD_API GrdCrypt(
		...
		void    *pKeyBuf,       // Encryption/decryption secret key for software-implemented algorithm (NULL if not used)
...
int GRD_API GrdHash(
		...
		DWORD   dwDataLng,      // Data length
		void    *pData,         // Data for hash calculation
		DWORD   dwMethod,       // GrdSC_First/GrdSC_Next/GrdSC_Last
		void    *pDigest,       // Pointer to memory allocated for hash on GrdSC_Last step
		void    *pKeyBuf,       // Hash calculation secret key for software-implemented algorithm (NULL if not used)
...
int GRD_API GrdEnCode( DWORD dwCnvType, void  *pKeyBuf, void *pData, DWORD dwLng );
...
int GRD_API GrdPI_Update(
		...
		void    *pData,             // Pointer to buffer with data to be written
...
int GRD_API GrdTRU_SetKey(
		HANDLE  hGrd,               // Handle to Guardant protected container
		void    *pKey128 );         // Unique Trusted Remote Update 128-bit secret key
...
int GRD_API GrdTRU_DecryptQuestionEx( 
        HANDLE  hGrd,               // Handle to Guardant protected container of dongle that contains
...
int GRD_API GrdTRU_DecryptQuestionTimeEx( 
		...
		DWORD   dwLngHash,          // Size for Hash
		void    *pHash,             // Pointer to Hash of previous
		DWORD   dwMode,             // GrdTRU_CryptMode_GSII64      : crypt=gsII64( 8 byte), hash=based on gsII64(8 byte)
                                    // GrdTRU_CryptMode_AES128SHA256: crypt=aes128(16 byte), hash=sha256(32 byte)
		DWORD   dwReserved,         // Reserved, must be 0
		void    *pReserved );       // Reserved, must be NULL
...
int GRD_API GrdTRU_EncryptAnswerEx(
		...
		void    *pData,             // Buffer for data to be written
		DWORD   dwLngQuestion,      // Size for Question
		void    *pQuestion,         // Pointer to decrypted Question
...
int GRD_API GrdTRU_EncryptAnswer(
		...
		DWORD   dwLng,              // Size of data to be written
		void    *pData,             // Buffer for data to be written
		void    *pQuestion,         // Pointer to decrypted Question            8 bytes (64 bit)
...
int GRD_API GrdTRU_ApplyAnswer(
		HANDLE  hGrd,               // Handle to Guardant protected container of dongle with corresponding pregenerated question
		void    *pAnswer,           // Answer data update buffer prepared and encrypted by GrdTRU_EncryptAnswer
...
int GRD_API GrdSign(
		...
		void    *pData,             // Pointer to message for sign
...
int GRD_API GrdPI_GetTimeLimit(
		HANDLE  hGrd,                         // Handle to Guardant protected container
...
int GRD_API GrdSplitSystemTime(
		HANDLE  hGrd,                         // Handle to Guardant protected container
		TGrdSystemTime *pGrdSystemTime,       // Pointer to source system time
...
int GRD_API GrdCodeLoad(
		...
		void    *pFileBuf,                    // Pointer to the buffer for GCEXE-file
		void    *pReserved );                 // Reserved
...
int GRD_API GrdRegisterDongleNotification(
		HANDLE  hGrd,                         // Handle to Guardant protected container
...
int GRD_API GrdUnRegisterDongleNotification(
		HANDLE hGrd );                        // Handle to Guardant protected container
